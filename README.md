...
- 클라이언트 화면에서 탭을 이용해 메뉴를 확인할 수 있습니다.
- 관리자 페이지에서 유저 회원 가입, 로그인을 할 수있습니다.
- 관리자 페이지에서 메뉴를 생성, 조회, 편집, 삭제할 수 있습니다.
- 관리자 페이지에서 전체 메뉴의 정보를 csv로 다운받을 수 있습니다.
...
### 파일을 폼과 함께 제출할때 문제 해결

파일을 폼과 함께 제출할 때 헤더를 enctype="multipart/form-data" 설정해줘야하고, 파일을 같이 받을때와 파일을 같이 받지 않을때의 코드를 설정해줘야한다.
